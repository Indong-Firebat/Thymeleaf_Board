...
public class BoardController extends UiUtils {
	...
	/* @ModalAttribute
	 * 파라미터로 전달받은 객체를 자동으로 뷰까지 전달이 가능
	 * 다시 파라미터를 BoardDTO 클래스로 변경하고 getBoardList메소드의 인자로 params를 전달 
	 * */
	@GetMapping(value = "/board/list.do")
	public String openBoardList(@ModelAttribute("params") BoardDTO params, Model model) {
		
		//BoardService에서 호출한 getBoardList 메소드의 실행결과를 list에 담아 뷰로 전달
		List<BoardDTO> boardList = boardService.getBoardList(params);
		//컨트롤러에서 뷰로 전달
		model.addAttribute("boardList", boardList);

		//src/main/resources의 templates/board/list.html을 의미
		return "board/list";
	}
	...
	public void downloadAttachFile(@RequestParam(value = "idx", required = false) final Long idx, Model model, HttpServletResponse response) {
		...
		try {
			...
			response.getOutputStream().close();//버퍼 정리
